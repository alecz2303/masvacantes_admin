    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($request->get('tipo') == 'c') {
                $query = $model::select($dataType->name.'.*')->whereNotIn('role_id', [1,3,4,5,6,7,8])->orWhereNull('role_id');
            } elseif ($request->get('tipo') == 'e') {
                $user = User::find(Auth::user()->id);
                if ($user->role_id == 1) {
                    $query = $model::select($dataType->name.'.*')->whereNotIn('role_id', [1,2,8]);
                } else {
